...
import { Building2, Calendar, CheckCircle2, XCircle, Clock, Sparkles, Save, AlertCircle, Tag, Image as ImageIcon, Upload } from 'lucide-react';
...
export const OwnerHomeView: React.FC<OwnerHomeViewProps> = ({ currentUser, halls, bookings, onUpdateHall, onUpdateBookingStatus, onCreatePost }) => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <section className="bg-white p-5 rounded-3xl border border-gray-200 space-y-4">
          ...
          {!isEditing && persistedHall ? (
            <div className="space-y-3"><div className="h-44 bg-gray-100 rounded-2xl overflow-hidden">{persistedHall.coverImage ? <img src={persistedHall.coverImage} className="w-full h-full object-cover" alt={persistedHall.name}/> : <div className="w-full h-full flex items-center justify-center text-gray-400"><ImageIcon/></div>}</div><h3 className="text-lg font-black">{persistedHall.name}</h3><p className="text-xs text-gray-600">{persistedHall.location} — {persistedHall.city}</p><p className="text-xs text-gray-600">السعر: {persistedHall.price.toLocaleString('ar-IQ')} د.ع • العربون: {persistedHall.deposit.toLocaleString('ar-IQ')} د.ع • السعة: {persistedHall.capacity}</p><p className="text-xs text-gray-600">{persistedHall.description}</p></div>
          ) : (
            <form onSubmit={saveHall} className="space-y-3">
              ...
              <div className="grid sm:grid-cols-2 gap-2">
                <div><FieldLabel>الصورة الرئيسية للقاعة</FieldLabel><label className="flex items-center justify-center gap-2 border border-dashed rounded-xl p-3 text-xs font-bold cursor-pointer bg-gray-50"><Upload className="w-4 h-4"/>{draft.coverImage ? 'تغيير الصورة من المعرض' : 'اختيار صورة من المعرض'}<input type="file" accept="image/*" className="hidden" onChange={(e)=>{const f=e.target.files?.[0]; if(f) void uploadImage(f,'cover');}}/></label>{draft.coverImage && <img src={draft.coverImage} alt="الصورة الرئيسية" className="mt-2 h-24 w-full object-cover rounded-xl"/>}</div>
                <div><FieldLabel>صورة حساب القاعة</FieldLabel><label className="flex items-center justify-center gap-2 border border-dashed rounded-xl p-3 text-xs font-bold cursor-pointer bg-gray-50"><Upload className="w-4 h-4"/>{draft.profileImageUrl ? 'تغيير صورة الحساب' : 'اختيار صورة الحساب'}<input type="file" accept="image/*" className="hidden" onChange={(e)=>{const f=e.target.files?.[0]; if(f) void uploadImage(f,'profile');}}/></label>{draft.profileImageUrl && <img src={draft.profileImageUrl} alt="صورة الحساب" className="mt-2 h-20 w-20 object-cover rounded-full mx-auto"/>}</div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2"><div><FieldLabel>السعر الكامل (د.ع)</FieldLabel><input type="number" min="0" value={draft.price || ''} onChange={(e)=>setField('price',Number(e.target.value))} placeholder="مثال: 3500000" className="w-full px-3 py-2 border rounded-xl text-xs" required/></div><div><FieldLabel>مبلغ العربون (د.ع)</FieldLabel><input type="number" min="0" value={draft.deposit || ''} onChange={(e)=>setField('deposit',Number(e.target.value))} placeholder="مثال: 500000" className="w-full px-3 py-2 border rounded-xl text-xs" required/></div><div><FieldLabel>سعة القاعة (شخص)</FieldLabel><input type="number" min="1" value={draft.capacity || ''} onChange={(e)=>setField('capacity',Number(e.target.value))} placeholder="مثال: 500" className="w-full px-3 py-2 border rounded-xl text-xs" required/></div></div>
              <div><FieldLabel>وصف القاعة</FieldLabel><textarea value={draft.description} onChange={(e)=>setField('description',e.target.value)} placeholder="اكتب تفاصيل القاعة والخدمات المتوفرة" className="w-full px-3 py-2 border rounded-xl text-xs h-20"/></div>
              <button disabled={isSaving||isUploading} className="w-full py-2.5 bg-emerald-800 disabled:bg-gray-400 text-white rounded-xl text-xs font-bold flex items-center justify-center gap-1"><Save className="w-4 h-4"/>{isUploading?'جاري رفع الصورة...':isSaving?'جاري الحفظ...':persistedHall?'حفظ التعديلات':'إنشاء صفحة القاعة'}</button>
            </form>
          )}
        </section>
        ...
        <section className="bg-white p-5 rounded-3xl border border-gray-200 space-y-3">
          ...
          {hallBookings.length===0 ? <div className="p-8 text-center text-xs text-gray-500 border border-dashed rounded-2xl">لا توجد حجوزات موجهة إلى قاعتك حالياً.</div> : hallBookings.map((booking)=><div key={booking.id} className="p-4 border rounded-2xl space-y-2"><div className="flex justify-between gap-2"><b className="text-xs">{booking.requesterName||booking.customerName}</b><span className="text-[11px]">{booking.status}</span></div><div className="text-[11px] text-gray-600">{booking.date} • {booking.startTime||booking.timeSlot} {booking.endTime?`- ${booking.endTime}`:''}</div>{(booking.status==='قيد المراجعة'||booking.status==='pending')&&<div className="flex gap-2"><button onClick={()=>onUpdateBookingStatus(booking.id,'مقبول')} className="px-3 py-2 bg-emerald-100 text-emerald-800 rounded-xl text-xs font-bold"><CheckCircle2 className="inline w-4 h-4 ml-1"/>قبول</button><button onClick={()=>onUpdateBookingStatus(booking.id,'مرفوض')} className="px-3 py-2 bg-rose-100 text-rose-800 rounded-xl text-xs font-bold"><XCircle className="inline w-4 h-4 ml-1"/>رفض</button></div>}</div>)}
        </section>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <form onSubmit={publishPost} className="bg-white p-5 rounded-3xl border space-y-3"><h2 className="font-bold flex gap-2"><Sparkles className="w-5 h-5 text-amber-600"/>نشر في Explore</h2><div><FieldLabel>عنوان المنشور</FieldLabel><input value={postTitle} onChange={(e)=>setPostTitle(e.target.value)} placeholder="عنوان المنشور" className="w-full px-3 py-2 border rounded-xl text-xs"/></div><div><FieldLabel>الوصف</FieldLabel><textarea value={postCaption} onChange={(e)=>setPostCaption(e.target.value)} placeholder="اكتب وصف المنشور" className="w-full px-3 py-2 border rounded-xl text-xs"/></div><div><FieldLabel>صورة المنشور</FieldLabel><label className="flex items-center justify-center gap-2 border border-dashed rounded-xl p-3 text-xs font-bold cursor-pointer bg-gray-50"><Upload className="w-4 h-4"/>{postMediaUrl?'تغيير الصورة':'اختيار صورة من المعرض'}<input type="file" accept="image/*" className="hidden" onChange={(e)=>{const f=e.target.files?.[0]; if(f) void uploadImage(f,'post');}}/></label>{postMediaUrl&&<img src={postMediaUrl} alt="المنشور" className="mt-2 h-28 w-full object-cover rounded-xl"/>}</div><button disabled={isUploading} className="px-4 py-2 bg-amber-600 disabled:bg-gray-400 text-white rounded-xl text-xs font-bold">نشر</button></form>

        <form onSubmit={publishOffer} className="bg-white p-5 rounded-3xl border space-y-3"><h2 className="font-bold flex gap-2"><Tag className="w-5 h-5 text-emerald-700"/>إنشاء عرض</h2><div><FieldLabel>عنوان العرض</FieldLabel><input value={offerTitle} onChange={(e)=>setOfferTitle(e.target.value)} placeholder="مثال: خصم الحجز المبكر" className="w-full px-3 py-2 border rounded-xl text-xs" required/></div><div><FieldLabel>تفاصيل العرض</FieldLabel><textarea value={offerDescription} onChange={(e)=>setOfferDescription(e.target.value)} placeholder="تفاصيل العرض" className="w-full px-3 py-2 border rounded-xl text-xs"/></div><div className="grid grid-cols-1 sm:grid-cols-3 gap-2"><div><FieldLabel>سعر العرض (د.ع)</FieldLabel><input type="number" min="0" value={offerPrice||''} onChange={(e)=>setOfferPrice(Number(e.target.value))} placeholder="سعر العرض" className="w-full px-2 py-2 border rounded-xl text-xs" required/></div><div><FieldLabel>بداية العرض</FieldLabel><input type="date" value={offerStart} onChange={(e)=>setOfferStart(e.target.value)} className="w-full px-2 py-2 border rounded-xl text-xs" required/></div><div><FieldLabel>نهاية العرض</FieldLabel><input type="date" value={offerEnd} onChange={(e)=>setOfferEnd(e.target.value)} className="w-full px-2 py-2 border rounded-xl text-xs" required/></div></div><button className="px-4 py-2 bg-emerald-700 text-white rounded-xl text-xs font-bold">حفظ العرض</button></form>
      </div>
    </div>
  );
};
