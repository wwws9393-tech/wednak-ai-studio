...
export const OfferDetailsModal: React.FC<Omit<OfferCardProps, 'onOpen'> & { onClose: () => void; onOpenOwner: () => void }> = ({ offer, ownerName, ownerImage, onClose, onOpenOwner }) => {
  ...
  return <div className="fixed inset-0 z-[130] bg-slate-950/75 backdrop-blur-sm grid place-items-center p-3" onClick={onClose}>
    <div className="w-full max-w-lg overflow-hidden rounded-3xl bg-white border border-lime-200 shadow-2xl" onClick={(event) => event.stopPropagation()} role="dialog" aria-modal="true">
      <header className="relative bg-gradient-to-l from-emerald-950 via-emerald-900 to-emerald-800 p-5 text-white"><button type="button" onClick={onClose} className="absolute left-4 top-4 w-10 h-10 rounded-full bg-white/10 grid place-items-center"><X className="w-5 h-5"/></button><span className="inline-flex rounded-full bg-amber-300 px-3 py-1 text-[10px] font-black text-emerald-950"><Percent className="w-3 h-3 ml-1"/>عرض ويدنك</span><h2 className="mt-4 text-xl font-black text-amber-100 pl-12">{offer.title}</h2></header>
