...
export const PushNotificationSettings: React.FC<{ userId: string }> = ({ userId }) => {
  ...
  useEffect(() => {
    ...
    refreshPushRegistration(userId).catch((error) => console.warn('Push refresh skipped:', error));
  }, [userId]);
  ...
  const enable = async () => {
    ...
    try {
      ...
      setMessage('تم تفعيل إشعارات الهاتف لهذا الحساب.');
    } catch (error) {
    ...
    <div className="rounded-2xl border border-emerald-100 bg-emerald-50/50 p-4 space-y-3">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="mt-0.5 text-[11px] text-gray-600">{active ? 'فعالة على هذا الجهاز وتصل حتى عند إغلاق ويدنك.' : 'فعّلها لاستلام طلبات وتحديثات الحجوزات خارج التطبيق.'}</p>
        </div>
      </div>
      <button type="button" disabled={busy} onClick={active ? disable : enable} className={`flex w-full items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-xs font-black text-white disabled:opacity-50 ${active ? 'bg-gray-700' : 'bg-emerald-700'}`}>
        ...
        {busy ? 'جاري التنفيذ…' : active ? 'إيقاف إشعارات هذا الجهاز' : 'تفعيل إشعارات الهاتف'}
      </button>
      {message && <p className={`flex items-center gap-1 text-[11px] font-bold ${message.startsWith('تم') ? 'text-emerald-700' : 'text-rose-700'}`}>{message.startsWith('تم') && <CheckCircle2 className="h-4 w-4" />}{message}</p>}
    </div>
  );
};
