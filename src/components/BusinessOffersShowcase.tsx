    ...
    <div className="grid gap-3 p-4 sm:grid-cols-2">
      {activeOffers.map((offer) => {
        ...
        return <button key={offer.id} type="button" onClick={() => setSelectedOffer(offer)} className="group rounded-2xl border border-lime-200 bg-white p-4 text-right shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">
          <div className="flex items-start justify-between gap-2"><span className="rounded-full bg-amber-100 px-2.5 py-1 text-[10px] font-black text-amber-900">عرض ويدنك</span>{discount > 0 && <span className="inline-flex items-center gap-1 text-xs font-black text-emerald-800"><Percent className="h-3.5 w-3.5" />{discount}%</span>}</div>
