    ...
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-2 sm:p-4 overflow-y-auto" id="provider-details-modal-overlay">
      <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[94vh] overflow-y-auto shadow-2xl border border-gray-200 flex flex-col justify-between my-auto">
        <div className="relative h-56 sm:h-72 w-full bg-black rounded-t-3xl overflow-visible mb-16">
          <img src={provider.coverImage || provider.avatar} alt={provider.name} className="w-full h-full object-cover" decoding="async" onError={(e) => { e.currentTarget.src = 'https://images.unsplash.com/photo-1537633552985-df8429e8048b?auto=format&fit=crop&w=1200&q=80'; }} />
        ...
        <div className="p-5 space-y-5" dir="rtl">
          ...
          {(portfolio.length > 0 || providerPosts.length > 0) && <div><h3 className="text-sm font-bold text-gray-900 mb-2 flex items-center gap-1.5"><Camera className="w-4 h-4 text-emerald-600" />معرض الأعمال ({portfolio.length + providerPosts.length})</h3><div className="grid grid-cols-2 sm:grid-cols-3 gap-3">{portfolio.map((url, idx) => <button onClick={()=>setViewing({url,type:isVideoUrl(url)?'video':'image',description:provider.portfolioDescriptions?.[url]})} key={`portfolio-${idx}`} className="text-right aspect-square rounded-xl overflow-hidden border bg-gray-100 relative"><MediaThumbnail url={url} type={isVideoUrl(url)?'video':'image'} thumbnailUrl={provider.portfolioThumbnails?.[url]} alt={`عمل ${idx + 1}`} /><span className="absolute bottom-0 inset-x-0 bg-black/60 text-white p-2 text-[10px]">{provider.portfolioDescriptions?.[url]||'بدون وصف'}</span></button>)}{providerPosts.map(post => <button onClick={()=>setViewing({url:post.mediaUrl,type:post.mediaType,title:post.title,description:post.caption})} key={post.id} className="text-right aspect-square rounded-xl overflow-hidden border bg-gray-100 relative"><MediaThumbnail url={post.mediaUrl} type={post.mediaType} thumbnailUrl={post.thumbnailUrl} alt={post.title} /><div className="absolute bottom-0 inset-x-0 p-2 bg-black/60 text-white text-[10px] line-clamp-1">{post.title}</div></button>)}</div></div>}
