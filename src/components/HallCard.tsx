...
import { Heart, Star, Users, MapPin, Sparkles, ArrowLeft, Building2, ShieldCheck } from 'lucide-react';
      ...
      <div className="relative h-48 sm:h-52 w-full overflow-hidden bg-gray-100 cursor-pointer" onClick={() => onSelectHall(hall)}>
        ...
        <div className="absolute bottom-3 right-3 left-3 flex items-center justify-between text-white text-xs">
          ...
          <div className="flex items-center gap-1 bg-amber-500 text-black px-2 py-0.5 rounded-md font-bold">
            ...
            <span className="text-[10px] text-gray-800">({hall.reviewsCount})</span>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 flex-1 flex flex-col justify-between">
        <div>
          <div className="flex items-start justify-between gap-2 mb-2">
            <h3 
              ...
              className="text-base font-bold text-gray-900 group-hover:text-emerald-800 transition-colors cursor-pointer line-clamp-1"
            >
              {hall.name}
            </h3>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 py-2 px-3 bg-amber-50/50 rounded-xl border border-amber-100/60 mb-3 text-xs">
            <div className="flex items-center gap-1.5 text-gray-700">
              ...
              <span>السعة: <strong className="text-gray-900">{hall.capacity} شخص</strong></span>
            </div>
            <div className="flex items-center gap-1.5 text-gray-700">
              <Sparkles className="w-3.5 h-3.5 text-amber-600" />
              <span>العربون: <strong className="text-emerald-800">{hall.depositFormatted}</strong></span>
            </div>
          </div>
        </div>
        ...
        <div className="pt-2 border-t border-gray-100 flex items-center justify-between mt-auto">
          <div>
            ...
            <span className="text-base font-black text-emerald-800">{hall.priceFormatted}</span>
          </div>

          <div className="flex items-center gap-2">
            <button
              onClick={() => onSelectHall(hall)}
              className="px-3 py-2 text-xs font-semibold text-emerald-800 bg-emerald-50 hover:bg-emerald-100 rounded-xl transition-colors"
              id={`details-btn-hall-${hall.id}`}
            >
              التفاصيل
            </button>
            {!isSelfHall ? (
              <button
                ...
                className="px-4 py-2 text-xs font-bold text-white bg-emerald-700 hover:bg-emerald-800 active:scale-95 rounded-xl shadow-xs transition-all flex items-center gap-1"
                id={`book-btn-hall-${hall.id}`}
              >
                <span>احجز الآن</span>
