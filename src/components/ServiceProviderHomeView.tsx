...
export const ServiceProviderHomeView: React.FC<ServiceProviderHomeViewProps> = (props) => {
  const { currentUser, serviceProviders, bookings, onUpdateBookingStatus, onCreatePost } = props;
  ...
  const saveProvider = async (event: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      const saved = await saveOwnedServiceProvider({ ...draft, ownerId: currentUser.id, portfolio: portfolioText.split('\n').map((v) => v.trim()).filter(Boolean), phone: currentUser.phone || draft.phone });
      setDraft(saved); notifyUpdated(saved); setIsEditing(false); setMessage('تم حفظ صفحة الخدمة في Firestore بنجاح.');
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6 dir-rtl" id="service-provider-home-dashboard">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <section className="bg-white p-5 rounded-3xl border border-gray-200 space-y-4">
          ...
          {!isEditing && persistedProvider ? <div className="space-y-3"><div className="h-44 bg-gray-100 rounded-2xl overflow-hidden">{persistedProvider.coverImage ? <img src={persistedProvider.coverImage} className="w-full h-full object-cover" alt={persistedProvider.name}/> : <div className="w-full h-full flex items-center justify-center text-gray-400"><ImageIcon/></div>}</div><h3 className="text-lg font-black">{persistedProvider.name}</h3><p className="text-xs text-gray-600">{persistedProvider.serviceCategory} — {persistedProvider.location}</p><p className="text-xs text-gray-600">{persistedProvider.description}</p>{persistedProvider.portfolio.length > 0 && <div className="grid grid-cols-3 gap-2">{persistedProvider.portfolio.slice(0,6).map((url) => <img key={url} src={url} alt="عمل" className="h-20 w-full object-cover rounded-xl bg-gray-100"/>)}</div>}</div> : <form onSubmit={saveProvider} className="space-y-3"><input value={draft.name} onChange={(e)=>setField('name', e.target.value)} placeholder="اسم الخدمة أو الاستوديو" className="w-full px-3 py-2 border rounded-xl text-xs" required/><select value={draft.serviceCategory} onChange={(e)=>setField('serviceCategory', e.target.value as ServiceCategory)} className="w-full px-3 py-2 border rounded-xl text-xs">{CATEGORIES.map((category)=><option key={category}>{category}</option>)}</select><div className="grid grid-cols-2 gap-2"><input value={draft.location} onChange={(e)=>setField('location',e.target.value)} placeholder="العنوان" className="px-3 py-2 border rounded-xl text-xs" required/><input value={draft.city} onChange={(e)=>setField('city',e.target.value)} placeholder="المحافظة" className="px-3 py-2 border rounded-xl text-xs" required/></div><input value={draft.coverImage} onChange={(e)=>setField('coverImage',e.target.value)} placeholder="رابط الصورة الرئيسية" className="w-full px-3 py-2 border rounded-xl text-xs dir-ltr"/><input value={draft.avatar} onChange={(e)=>setField('avatar',e.target.value)} placeholder="رابط صورة الحساب" className="w-full px-3 py-2 border rounded-xl text-xs dir-ltr"/><input type="number" value={draft.priceStart} onChange={(e)=>setField('priceStart',Number(e.target.value))} placeholder="السعر الابتدائي" className="w-full px-3 py-2 border rounded-xl text-xs"/><textarea value={draft.description} onChange={(e)=>setField('description',e.target.value)} placeholder="وصف الخدمة" className="w-full px-3 py-2 border rounded-xl text-xs h-20"/><textarea value={portfolioText} onChange={(e)=>setPortfolioText(e.target.value)} placeholder="روابط صور الأعمال، كل رابط بسطر" className="w-full px-3 py-2 border rounded-xl text-xs h-24 dir-ltr"/><button disabled={isSaving} className="w-full py-2.5 bg-emerald-800 text-white rounded-xl text-xs font-bold flex items-center justify-center gap-1"><Save className="w-4 h-4"/>{isSaving ? 'جاري الحفظ...' : persistedProvider ? 'حفظ التعديلات' : 'إنشاء صفحة الخدمة'}</button></form>}
        </section>
        <section className="bg-white p-5 rounded-3xl border border-gray-200 space-y-3"><h2 className="font-bold flex items-center gap-2"><Calendar className="w-5 h-5 text-emerald-700"/>الحجوزات الواردة ({providerBookings.length})</h2>{providerBookings.length === 0 ? <div className="p-8 text-center text-xs text-gray-500 border border-dashed rounded-2xl">لا توجد حجوزات موجهة إلى خدمتك حالياً.</div> : providerBookings.map((booking)=><div key={booking.id} className="p-4 border rounded-2xl space-y-2"><div className="flex justify-between gap-2"><b className="text-xs">{booking.requesterName || booking.customerName}</b><span className="text-[11px]">{booking.status}</span></div><div className="text-[11px] text-gray-600">{booking.date} • {booking.startTime || booking.timeSlot} {booking.endTime ? `- ${booking.endTime}` : ''}</div>{(booking.status === 'قيد المراجعة' || booking.status === 'pending') && <div className="flex gap-2"><button onClick={()=>onUpdateBookingStatus(booking.id,'مقبول')} className="px-3 py-2 bg-emerald-100 text-emerald-800 rounded-xl text-xs font-bold"><CheckCircle2 className="inline w-4 h-4 ml-1"/>قبول</button><button onClick={()=>onUpdateBookingStatus(booking.id,'مرفوض')} className="px-3 py-2 bg-rose-100 text-rose-800 rounded-xl text-xs font-bold"><XCircle className="inline w-4 h-4 ml-1"/>رفض</button></div>}</div>)}</section>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6"><form onSubmit={publishPost} className="bg-white p-5 rounded-3xl border space-y-3"><h2 className="font-bold flex gap-2"><Sparkles className="w-5 h-5 text-amber-600"/>نشر أعمالك في Explore</h2><input value={postTitle} onChange={(e)=>setPostTitle(e.target.value)} placeholder="عنوان المنشور" className="w-full px-3 py-2 border rounded-xl text-xs"/><textarea value={postCaption} onChange={(e)=>setPostCaption(e.target.value)} placeholder="الوصف" className="w-full px-3 py-2 border rounded-xl text-xs"/><input value={postMediaUrl} onChange={(e)=>setPostMediaUrl(e.target.value)} placeholder="رابط الصورة أو الوسائط" className="w-full px-3 py-2 border rounded-xl text-xs dir-ltr"/><button className="px-4 py-2 bg-amber-600 text-white rounded-xl text-xs font-bold">نشر</button></form><form onSubmit={publishOffer} className="bg-white p-5 rounded-3xl border space-y-3"><h2 className="font-bold flex gap-2"><Tag className="w-5 h-5 text-emerald-700"/>إنشاء عرض</h2><input value={offerTitle} onChange={(e)=>setOfferTitle(e.target.value)} placeholder="عنوان العرض" className="w-full px-3 py-2 border rounded-xl text-xs"/><textarea value={offerDescription} onChange={(e)=>setOfferDescription(e.target.value)} placeholder="تفاصيل العرض" className="w-full px-3 py-2 border rounded-xl text-xs"/><div className="grid grid-cols-3 gap-2"><input type="number" value={offerPrice} onChange={(e)=>setOfferPrice(Number(e.target.value))} placeholder="سعر العرض" className="px-2 py-2 border rounded-xl text-xs"/><input type="date" value={offerStart} onChange={(e)=>setOfferStart(e.target.value)} className="px-2 py-2 border rounded-xl text-xs"/><input type="date" value={offerEnd} onChange={(e)=>setOfferEnd(e.target.value)} className="px-2 py-2 border rounded-xl text-xs"/></div><button className="px-4 py-2 bg-emerald-700 text-white rounded-xl text-xs font-bold">حفظ العرض</button></form></div>
    </div>
  );
};
