...
import { Hall, ServiceProvider, FeedPost, Booking, Complaint, AppNotification, UserProfile, BookingStatus, BusinessOffer } from './types';
...
import { subscribeBusinessOffers } from './lib/business';
...
function readLegacyBookings(): Booking[] {
  try {
    const value=JSON.parse(localStorage.getItem('wednak_bookings')||'[]');
    return Array.isArray(value)?value.map((b:any,index:number)=>{const id=String(b.id||b.bookingId||`legacy-booking-${index}`);return {...b,id,requesterId:b.requesterId||b.customerId,targetOwnerId:b.targetOwnerId||b.ownerId,bookingId:b.bookingId||id}}):[];
  } catch { return []; }
}
...
export function App() {
  ...
  useEffect(() => {
    ...
    const unsubOffers = subscribeBusinessOffers(setOffers);
    return () => { unsubHalls(); unsubProviders(); unsubPosts(); unsubOffers(); };
  }, []);
  ...
  const validFavoriteIds = Array.from(new Set(favoriteIds.filter((id) => halls.some((h) => h.id === id) || serviceProviders.some((p) => p.id === id))));
  ...
  const renderRoleSpecificView = () => {
    ...
    switch (currentTab) {
      ...
      case 'profile': return <ProfileView currentUser={currentUser} halls={halls} serviceProviders={serviceProviders} onUpdateProfile={handleUpdateProfile} onSelectTab={setCurrentTab} onOpenPrivacyModal={()=>setActiveLegalModal('privacy')} onOpenTermsModal={()=>setActiveLegalModal('terms')} onOpenSupportModal={()=>setActiveLegalModal('support')} onOpenAuthModal={()=>setIsAuthModalOpen(true)}/>;
      default: return null;
    }
  };
